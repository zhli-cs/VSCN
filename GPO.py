...
class GPO(nn.Module):
    ...
    def compute_pool_weights(self, lengths, features, bsize):
        max_len = lengths
        pe_max_len = self.get_pe(max_len)
        pes = pe_max_len.unsqueeze(0).repeat(bsize, 1, 1).cuda()
        mask = torch.arange(max_len).expand(bsize, max_len).cuda()
        mask = (mask < lengths).unsqueeze(-1)
        pes = pes.masked_fill(mask == 0, 0)

        self.gru.flatten_parameters()
        lengths = torch.tensor(lengths).unsqueeze(0).repeat(bsize)
    ...
    def forward(self, features, lengths):
        ...
        pool_weights, mask = self.compute_pool_weights(lengths, features, bsize)

        features = features[:, :int(lengths), :]
